...
def SVM_WD_Train_Test(X_train,y_train,X_test_FRR,y_test_FRR,X_test_FAR,y_test_FAR):
    ...
    #max_X_test_FRR=X_test_FRR.max(axis=0)
    #X_test_FRR=(X_test_FRR)/max_X_test_FRR
    #inds1 = np.where(np.isnan(X_test_FRR))
    #Place column means in the indices. Align the arrays using take
    #X_test_FRR[inds1] = np.take(0, inds1[1])

    
    #max_X_test_FAR=X_test_FAR.max(axis=0)
    #X_test_FAR=X_test_FAR/max_X_test_FAR
    #inds2 = np.where(np.isnan(X_test_FAR))
    #Place column means in the indices. Align the arrays using take
    #X_test_FAR[inds2] = np.take(0, inds2[1])
